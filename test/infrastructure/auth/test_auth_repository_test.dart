...
main() {
  ...
  test('test if getSignedInUser returns null if no user is connected',
      () async {
    var user = await testRepo.getSignedInUser();
    expect(user.fold(() => unit, (u) => u), unit);
  });

  test('test if getSignedInUser returns user if a user is connected', () async {
    testRepo.signInWithEmailAndPassword(
        emailAddress: validTestEmail, password: validTestPassword);
    var user = await testRepo.getSignedInUser();
    expect(user.fold(() => unit, (u) => u), isA<User>());
  });
}
