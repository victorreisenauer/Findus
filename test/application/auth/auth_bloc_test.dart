...
void main() {
  ...
  User _currentUser =
      User(emailAddress: _emailAddress, name: _userName, id: _testId);

  group('AuthBloc =>', () {
    test('emits initial state on initialization"', () {
      final bloc = AuthBloc(mockApiAuthRepo);
      expectLater(bloc, emits(AuthState.initial()));
      bloc.close();
    });
    ...
    test('emits state "Unauthenticated" when no user is signed in', () {
      ...
      expectLater(bloc,
          emitsInOrder([AuthState.initial(), AuthState.unauthenticated()]));
      bloc.close();
    });
    ...
    test('emits state "Authenticated" with user obj when user is signed in',
      ...
      expectLater(
          bloc,
          emitsInOrder(
              [AuthState.initial(), AuthState.authenticated(_currentUser)]));
      bloc.close();
    });
