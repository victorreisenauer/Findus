import 'package:lrs_app_v3/domain/auth/validated_value_objects.dart';
...
void main() {
  group('All unit tests for EmailAddress ValueObject =>', () {
    ...
    test(
        'return ValueFailures if inputs are empty, missing an @, or missing a domain',
        () async {
      ...
      expect(failure1.fold((f) => f.failedValue, (_) => null), emptyInput);
      expect(failure2.fold((f) => f.failedValue, (_) => null), missingAtInput);
      expect(
          failure3.fold((f) => f.failedValue, (_) => null), missingDomainInput);
      expect(valid.fold((f) => null, (output) => output), validInput);
    });
  });
  ...
  group('Unit tests for Password ValueObject =>', () {
    ...
    test('return ValueFailures password is empty', () async {
      ...
      expect(failure1.fold((f) => f.failedValue, (_) => null), emptyPassword);
    });
    test('return ValueFailures password is shorter than 8 chars', () async {
      var failure1 = EmailAddress(shortPassword).value;
      expect(failure1.fold((f) => f.failedValue, (_) => null), shortPassword);
    });
    ...
    test('return password  if password is valid', () async {
      ...
      expect(valid.fold((f) => null, (password) => password), validPassword);
    });
  });
}
